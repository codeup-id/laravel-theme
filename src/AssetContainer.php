...
class AssetContainer {
    ...
    {
        if(!is_array($name)) {
            if(!isset($source)) throw new \ErrorException("Missing argument 2 for LaravelUP\Theme\AssetContainer::add()", 1);
            
            return $this->added($name, $source, $dependencies, $attributes);
        }

        foreach ($name as $array) {
            if(count($array) < 2) throw new \ErrorException("Missing value 2 of the array for LaravelUP\Theme\AssetContainer::add()", 1);
            $container = $array[0];
            $source = $array[1]; 
